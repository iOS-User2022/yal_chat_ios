...
final class VoIPTokenStore {
    private let subject: CurrentValueSubject<String?, Never>
    private let queue = DispatchQueue(label: "VoIPTokenStore.queue", qos: .userInitiated)
    ...
    init() {
        let stored = Storage.get(for: .voipToken, type: .userDefaults, as: String.self)
        self.subject = CurrentValueSubject<String?, Never>(stored)
    }
    ...
    func update(hexString: String) {
        queue.async { [weak self] in
            guard let self else { return }
            guard self.subject.value != hexString else { return } // de-dupe
            Storage.save(hexString, for: .voipToken, type: .userDefaults)
            DispatchQueue.main.async { self.subject.send(hexString) }
        }
    }
    ...
    func clear() {
        queue.async { [weak self] in
            guard let self else { return }
            Storage.delete(.voipToken, type: .userDefaults)
            DispatchQueue.main.async { self.subject.send(nil) }
        }
    }
